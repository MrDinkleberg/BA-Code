public class SegmentHeader {
    ...
    public int findFittingBlockList(long size){
        ...
        if(size >= 12 && size <= 23 && freeblocks[0] != 0 ) return 0;     //Die ersten Freispeicherlisten besitzen ein anderes Groessenintervall
        if(size >= 24 && size <= 35 && freeblocks[1] != 0 ) return 1;     //als die anderen
        if(size >= 36 && size <= 47 && freeblocks[2] != 0 ) return 2;
        if(size >= 48 && size <= 63 && freeblocks[3] != 0 ) return 3;
